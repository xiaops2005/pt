...
public class ClasspathPackageScanner {
	...
	/**
	 * 扫描指定包下面的实体注解javax.persistence.Entity
	 * @param basePackage
	 * @param nameList
	 * @return
	 * @throws IOException
	 */
	public static void doScan(String basePackage, List<String> nameList) {
							...
							if (name.startsWith(packageDirName)) {
								...
								if ((idx != -1) || recursive) {
									...
									if (name.endsWith(".class") && !entry.isDirectory()) {
										...
										//添加到classes  
										String quPath = basePackage + '.' + className;
										if (isContainEntity(quPath)) {
											nameList.add(quPath);
										}
									}
								}
	...
	*/
	public static void findAndAddClassesInPackageByFile(String packageName, String packagePath,
		...
		for (File file : dirfiles) {
			...
			if (file.isDirectory()) {
				...
				//添加到集合中去 
				String quPath = packageName + '.' + className;
				if (isContainEntity(quPath)) {
					nameList.add(quPath);
				}
			}
	 ...
	 * @return
	 */
	private static boolean isContainEntity(String classPath) {
		logger.info("扫描注解包："+classPath);
		boolean isEntity = false;
		try {
			Class<?> entityClassName = Class.forName(classPath);
			if (entityClassName.isAnnotationPresent(javax.persistence.Entity.class)) {
				isEntity = true;
			}
		} catch (Exception e) {
			logger.error("扫描实体包出现错误", e);
			throw new DataException("扫描实体包出现错误", e);
		}
		return isEntity;
	}
